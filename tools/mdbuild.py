#!/usr/bin/env python2
"""
A tool to convert docs markdown -> html
"""
...
doc_template = ''
doc_nav = ''
default_title = 'OpenStack Installer'


def getargs():
    d_text = """This version of mdbuild is specifically designed for use
                with the OpenStack installer documentation at
                https://github.com/Ubuntu-Solutions-Engineering/openstack-installer-docs"""
    parser = argparse.ArgumentParser(description=d_text)
    parser.add_argument(
        '--file', nargs=1, dest='file', help="process single file")
    parser.add_argument(
        '--source', nargs=1, default='./guides/', help="source directory")
    parser.add_argument(
...
def main():
    ...
    global args
    args = getargs()
    t = codecs.open(os.path.join(args.source, '../tools/huc.tpl'), encoding='utf-8')
    doc_template = t.read()
    t.close()
    #t = codecs.open(
